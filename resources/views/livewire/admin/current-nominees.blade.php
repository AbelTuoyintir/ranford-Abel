  ...
  <body class="bg-gray-50 text-gray-800 min-h-screen">
      ...
      <div class="container mx-auto px-4 py-4">
        <div class="flex justify-between items-center">
          ...
          <div class="flex items-center gap-4">
            <div class="relative">
              <input type="text" placeholder="Search nominees..." class="pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
    ...
    <div class="container mx-auto px-4 py-8">
      <div class="mb-8 flex justify-between items-center">
        <div>
          <h2 class="text-xl font-semibold text-gray-300">Current Nominees</h2>
          <p class="text-gray-300">Manage all nominees and their status</p>
        </div>
        <div class="flex items-center gap-3">
          <div class="flex items-center space-x-2">
            <label for="filter" class="text-sm font-medium text-gray-300">Filter by:</label>
          ...
          <div class="flex items-center space-x-2">
            <label for="sort" class="text-sm font-medium text-gray-300">Sort by:</label>
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 relative card-hover transition-all duration-300">
          <!-- Status Badge -->
          <span class="status-badge bg-{{ $nominee->status === 'submitted' ? 'green' : ($nominee->status === 'nominee' ? 'red' : 'yellow') }}-100 text-{{ $nominee->status === 'aspirant' ? 'green' : ($nominee->status === 'candidate' ? 'red' : 'yellow') }}-800">
          ...
          <div class="flex items-center gap-4 mb-4">
              ...
              <div>
                  ...
                  <div class="flex items-center mt-1 text-yellow-500">
                      @php
                          $rating = min(5, max(1, round($nominee->nominee_cgpa)));
                      @endphp
          ...
          <!-- Progress Bar (example using status) -->
          <div class="mb-4">
              <div class="flex justify-between text-xs text-gray-500 mb-1">
                  ...
                  <span>
                      @php
                          $progress = 0;
                          if($nominee->status === 'applicant') $progress = 10;
                          elseif($nominee->role === 'nominee') $progress = 25;
                          elseif($nominee->role === 'approved') $progress = 50;
                          elseif($nominee->role === 'aspirant') $progress = 75;
                          elseif($nominee->role === 'candidate') $progress = 100;
                      @endphp
                      {{ $progress }}%
                  </span>
              </div>
          ...
          <div class="space-y-2">
            ...
            <!-- Disqualify Button (now triggers modal) -->
