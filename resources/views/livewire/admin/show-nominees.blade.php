         ...
         <div class="container">
             @if ($errors->any())
                 <div class="fixed top-4 right-4 z-50 error-prompt" role="alert">
                     <div
                         class="bg-red-500 text-white px-6 py-4 rounded-lg shadow-xl transition-transform duration-300 ease-in-out animate-bounce">
             ...
             @if (session('success'))
                 <div class="fixed top-4 left-4 z-50 success-prompt" role="alert">
                     <div
                         class="bg-green-500 text-white px-6 py-4 rounded-lg shadow-xl transition-transform duration-300 ease-in-out animate-bounce">
                         <div class="flex items-center">
                             <svg class="w-6 h-6 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                 xmlns="http://www.w3.org/2000/svg">
                             ...
                             <form action="{{ route('nominees.disqualify', $nominee->id) }}" method="POST">
                                 ...
                                 <div class="mb-4">
                                     <label for="rejection_reason"
                                         class="block text-sm font-medium text-gray-700 text-left mb-1">Reason for
                                 ...
                                 <div class="items-center px-4 py-3">
                                     ...
                                     <button type="button" onclick="closeDisqualifyModal()"
                                         class="ml-2 px-4 py-2 bg-gray-200 text-gray-800 text-base font-medium rounded-md shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-300">
                                         Cancel
                                     </button>
                                 </div>
                             </form>
         ...
         <div class="min-h-screen bg-gray-50 py-8">
             <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                 ...
                 <div class="bg-white shadow rounded-lg overflow-hidden">
                     <!-- Status Banner -->
                     <div
                         class="bg-{{ $nominee->status === 'approved' ? 'green' : ($nominee->status === 'rejected' ? 'red' : 'blue') }}-500 px-4 py-2">
                         ...
                         <div class="flex flex-col md:flex-row">
                             ...
                             <div class="md:w-1/4 flex flex-col items-center">
                                 <div class="relative mb-4">
                                     ...
                                     <span
                                         class="absolute bottom-2 right-2 w-5 h-5 bg-{{ $nominee->verified ? 'green' : 'red' }}-500 rounded-full border-2 border-white"></span>
                                 </div>
                             ...
                             <div class="md:w-3/4 md:pl-8 mt-4 md:mt-0">
                                 ...
                                 <div>
                                     <label for="portfolio" class="text-xs font-medium text-gray-500">Change
                                         Portfolio</label>
                                     <form action="{{ route('nominees.updatePortfolio', $nominee->id) }}" method="POST"
                                         class="mt-1">
                                         @csrf
                                         @method('PATCH')
                                         <div>
                                             <label class="block text-sm font-medium text-gray-700 mb-1">Portfolio:</label>
                                             <select name="position" id="portfolio"
                                                 class="w-full p-2 border border-gray-300 rounded-md" required>
                                                 <option value="">Select Portfolio</option>
                                                 <optgroup label="SRC Positions">
                                                     <option value="SRC President"
                                                         {{ $nominee->position == 'SRC President' ? 'selected' : '' }}>SRC
                                                         President</option>
                                                     <option value="SRC Secretary"
                                                         {{ $nominee->position == 'SRC Secretary' ? 'selected' : '' }}>SRC
                                                         Secretary</option>
                                                     <option value="SRC Treasurer"
                                                         {{ $nominee->position == 'SRC Treasurer' ? 'selected' : '' }}>SRC
                                                         Treasurer</option>
                                                     <option value="SRC General Sports Secretary"
                                                         {{ $nominee->position == 'SRC General Sports Secretary' ? 'selected' : '' }}>
                                                         SRC General Sports Secretary</option>
                                                     <option value="SRC Coordinating Secretary"
                                                         {{ $nominee->position == 'SRC Coordinating Secretary' ? 'selected' : '' }}>
                                                         SRC Coordinating Secretary</option>
                                                     <option value="SRC Public Relations Officer"
                                                         {{ $nominee->position == 'SRC Public Relations Officer' ? 'selected' : '' }}>
                                                         SRC Public Relations Officer</option>
                                                     <option value="SRC Women's Commissioner"
                                                         {{ $nominee->position == "SRC Women's Commissioner" ? 'selected' : '' }}>
                                                         SRC Women's Commissioner</option>
                                                 </optgroup>

                                                 <optgroup label="Local NUGS Positions">
                                                     <option value="Local NUGS President"
                                                         {{ $nominee->position == 'Local NUGS President' ? 'selected' : '' }}>
                                                         Local NUGS President</option>
                                                     <option value="Local NUGS Secretary"
                                                         {{ $nominee->position == 'Local NUGS Secretary' ? 'selected' : '' }}>
                                                         Local NUGS Secretary</option>
                                                 </optgroup>

                                                 <optgroup label="JCRC Positions">
                                                     <option value="JCRC President"
                                                         {{ $nominee->position == 'JCRC President' ? 'selected' : '' }}>
                                                         JCRC President</option>
                                                     <option value="JCRC Secretary"
                                                         {{ $nominee->position == 'JCRC Secretary' ? 'selected' : '' }}>
                                                         JCRC Secretary</option>
                                                     <option value="JCRC Treasurer"
                                                         {{ $nominee->position == 'JCRC Treasurer' ? 'selected' : '' }}>
                                                         JCRC Treasurer</option>
                                                     <option value="JCRC Welfare Chairperson"
                                                         {{ $nominee->position == 'JCRC Welfare Chairperson' ? 'selected' : '' }}>
                                                         JCRC Welfare Chairperson</option>
                                                     <option value="JCRC Sport Secretary"
                                                         {{ $nominee->position == 'JCRC Sport Secretary' ? 'selected' : '' }}>
                                                         JCRC Sport Secretary</option>
                                                     <option value="JCRC Library Chairperson"
                                                         {{ $nominee->position == 'JCRC Library Chairperson' ? 'selected' : '' }}>
                                                         JCRC Library Chairperson</option>
                                                     <option value="JCRC SRC Representative(s)"
                                                         {{ $nominee->position == 'JCRC SRC Representative(s)' ? 'selected' : '' }}>
                                                         JCRC SRC Representative(s)</option>
                                                 </optgroup>

                                                 <optgroup label="GRASAG Positions">
                                                     <option value="GRASAG President"
                                                         {{ $nominee->position == 'GRASAG President' ? 'selected' : '' }}>
                                                         GRASAG President</option>
                                                     <option value="GRASAG Secretary"
                                                         {{ $nominee->position == 'GRASAG Secretary' ? 'selected' : '' }}>
                                                         GRASAG Secretary</option>
                                                     <option value="GRASAG Treasurer"
                                                         {{ $nominee->position == 'GRASAG Treasurer' ? 'selected' : '' }}>
                                                         GRASAG Treasurer</option>
                                                     <option value="GRASAG Financial Secretary"
                                                         {{ $nominee->position == 'GRASAG Financial Secretary' ? 'selected' : '' }}>
                                                         GRASAG Financial Secretary</option>
                                                     <option value="GRASAG Organising Secretary"
                                                         {{ $nominee->position == 'GRASAG Organising Secretary' ? 'selected' : '' }}>
                                                         GRASAG Organising Secretary</option>
                                                     <option value="GRASAG Women's Commissioner"
                                                         {{ $nominee->position == "GRASAG Women's Commissioner" ? 'selected' : '' }}>
                                                         GRASAG Women's Commissioner</option>
                                                 </optgroup>
                                             </select>
                                         </div>
                                         <button type="submit"
                         ...
                         <div class="px-6 py-4 border-b">
                             ...
                             <div class="bg-blue-50 rounded-lg p-4">
                                 <div class="flex items-center">
                                     ...
                                     <div>
                                         <h4 class="font-semibold">{{ $nominee->runningMate->running_mates_full_name }}
                                         </h4>
                                         <p class="text-sm text-gray-600">
                                             {{ $nominee->runningMate->running_mates_reg_number }}</p>
                                         <p class="text-sm text-gray-600">
                                             {{ $nominee->runningMate->running_mates_program }}</p>
                                         <p class="text-sm text-gray-600">cgpa •
                                             {{ $nominee->runningMate->running_mates_cgpa }}</p>
                                     </div>
                                 </div>
                     ...
                     <!-- Supporters Section -->
                     <div class="px-6 py-4">
                         <h3 class="text-lg font-medium text-gray-900 mb-4">GUARANTORS
                             ({{ $nominee->supporters->count() }})</h3>
                         ...
                         @foreach ($nominee->documents as $document)
                             <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                 <div class="flex items-center justify-between mb-2">
                                     ...
                                     <span
                                         class="text-xs px-2 py-1 rounded-full {{ $document->verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                 ...
                 <div class="mt-6 flex flex-col sm:flex-row gap-2">
                     @if ($nominee->status == 'rejected')
                         <form action="{{ route('nominees.requalify', $nominee->id) }}" method="POST" class="flex-1">
                             ...
                             <button type="submit"
                                 class=" h-full px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-md shadow flex items-center justify-center">
                     ...
                     @if ($nominee->status !== 'rejected')
                         <button onclick="openDisqualifyModal()"
                             class="h-full px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-md shadow flex items-center justify-center">
                             Disqualify
                         </button>

                         {{-- Check if the role is NOT candidate before showing the form --}}
                         @if ($nominee->role !== 'candidate')
                             <form action="{{ route('nominees.promote', $nominee->id) }}" method="POST"
                                 class="flex-1">
                                 @csrf
                                 {{-- If your route is defined as PUT/PATCH in web.php, uncomment the line below --}}
                                 {{-- @method('PATCH') --}}

                                 <input type="hidden" name="role" value="{{ match ($nominee->role) {
                                     'nominee' => 'aspirant',
                                     'aspirant' => 'candidate',
                                 } }}">
                         ...
                         @else
                             {{-- Optional: Show a badge or text indicating they've reached the final stage --}}
                             <span class="text-gray-500 italic px-4 py-3">Final Phase Reached</span>
                         @endif
                     @endif

                     <a href="{{ route('nomination-management') }}"
                         class="h-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md shadow flex items-center justify-center">
                         Back to List
                     </a>
                 </div>
             </div>
